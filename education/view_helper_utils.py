...
def get_aggregated_report_for_district(locations, time_range, config_list):
    ...
    for config in config_list:
        if config.get('collective_dict_key') in indicator_list:
            ...
            # get enroll list
            enroll_indicator_totals = get_numeric_data_all_schools_for_locations(enrollment_polls, locations,
                                                                                 term_range)
            ...
            for week in time_range:
                ...
                for school in schoolSource:
                    ...
                    if school.id in enroll_indicator_totals:
                        ...
                        school_absenteeism_percent_values[school.name][
                            config.get('collective_dict_key')] += compute_absent_values(school_attendance,
                                                                                        school_enrollment)
            ...
            for week in time_range:
                present_data_totals, no_data_totals = get_count_for_yes_no_response_by_school(attendance_polls,
                                                                                              locations, week)
                schools_that_responded = len(present_data_totals) + len(no_data_totals)

                if (school_with_no_zero_by_indicator[config.get('collective_dict_key')]):
        ...
        for school, school_absenteeism_percent in school_absenteeism_percent_values.items():
            school_absenteeism_percent_values[school][config.get('collective_dict_key')] = round(
                school_absenteeism_percent[config.get('collective_dict_key')] / len(time_range), 2)
...
def get_aggregated_report_for_district_single_indicator(locations, time_range, config_list):
    ...
    if config_list[0].get('collective_dict_key') not in ['Male Head Teachers', 'Female Head Teachers', 'Head Teachers']:
        ...
        for week in time_range:
            ...
            for school in schoolSource:
                ...
                if school.id in enroll_indicator_totals:
                    school_enrollment = enroll_indicator_totals[school.id]
                    school_attendance = attendance_data_totals[school.id] if school.id in attendance_data_totals else 0
                    school_absenteeism_percent_values[school.name]['absenteeism'] += compute_absent_values(
                        school_attendance, school_enrollment)
        ...
        for week in time_range:
            present_data_totals_schools = gendered_text_responses_by_school(week, locations, ['Yes', 'YES', 'yes'],
                                                                            config_list[0].get('gender'))
            weekly_present_result.append(sum(present_data_totals_schools.values()))
            absent_data_totals_schools = gendered_text_responses_by_school(week, locations, ['No', 'NO', 'no'],
                                                                           config_list[0].get('gender'))
            ...
            weekly_school_absenteeism(present_data_totals_schools, enroll_indicator_totals, schoolSource, school_absenteeism_percent_values)
    else:
        headteachersSource = EmisReporter.objects.filter(reporting_location__in=locations,
                                                         groups__name="Head Teachers").exclude(
            schools=None).select_related()
        ...
        for week in time_range:
            present_data_totals, no_data_totals = get_count_for_yes_no_response_by_school(attendance_polls, locations,
                                                                                          week)
    ...
    for school_name, school_dict in school_absenteeism_percent_values.items():
        collective_result[school_name] = {indicator: (round(school_dict['absenteeism'] / len(time_range), 2)),
    ...
    for config in config_list:
        if config.get('collective_dict_key') in indicator_list:
            ...
            for week in time_range:
                ...
                schools_that_responded_data = get_numeric_data_all_schools_for_locations(attendance_polls, locations,
                                                                                         week)
                ...
                for location in locations:
                    ...
                    if location.id in enroll_indicator_totals:
                        ...
                        location_attendance = attendance_data_totals[
                            location.id] if location.id in attendance_data_totals else 0
                        location_absenteeism_percent_values[location.name][
                            config.get('collective_dict_key')] += compute_absent_values(location_attendance,
                                                                                        location_enrollment)
            ...
            weekly_school_count = []
            for week in time_range:
                present_data_totals, absent_data_totals = get_count_for_yes_no_response_by_location(attendance_polls,
                                                                                                    locations, week)
                ...
                for location in locations:
                    ...
                    if location.id in enroll_indicator_totals:
                        location_enrollment = enroll_indicator_totals[location.id]
                        location_attendance = present_data_totals[
                            location.id] if location.id in present_data_totals else 0
                        location_absenteeism_percent_values[location.name][
                            config.get('collective_dict_key')] += compute_absent_values(location_attendance,
                                                                                        location_enrollment)
        ...
        for location, location_absenteeism_percent in location_absenteeism_percent_values.items():
            location_absenteeism_percent_values[location][config.get('collective_dict_key')] = round(
                location_absenteeism_percent[config.get('collective_dict_key')] / len(time_range), 2)
...
def get_aggregated_report_data_single_indicator(locations, time_range, config_list):
        ...
        for week in time_range:
            ...
            for location in locations:
                ...
                if location.id in enroll_indicator_totals:
                    ...
                    location_attendance = attendance_data_totals[
                        location.id] if location.id in attendance_data_totals else 0
                    location_absenteeism_percent_values[location.name] += compute_absent_values(location_attendance,
                                                                                                location_enrollment)

    elif config_list[0].get('collective_dict_key') in ['Male Head Teachers', 'Female Head Teachers']:
        enroll_indicator_totals = get_count_gender_deployed_head_teachers_by_location(headteachersSource,
                                                                                      config_list[0].get('gender'))
        totalCount = sum(enroll_indicator_totals.values())

        for week in time_range:
            present_data_totals = gendered_text_responses_by_location(week, locations, ['Yes', 'YES', 'yes'],
                                                                      config_list[0].get('gender'))
            weekly_present_result.append(sum(present_data_totals.values()))
            absent_data_totals = gendered_text_responses_by_location(week, locations, ['No', 'NO', 'no'],
                                                                     config_list[0].get('gender'))
    ...
    for location, location_absenteeism_percent in location_absenteeism_percent_values.items():
        collective_result[config_list[0].get('collective_dict_key')].append(
            {location: round(location_absenteeism_percent / len(time_range), 2)})
