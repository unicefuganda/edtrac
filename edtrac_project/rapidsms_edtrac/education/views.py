...
def get_mode_progress(mode):
    try:
        mode_progress = (100 * sorted(target.values()).index(mode) + 1) / float(
            len(target.keys())) # offset zero-based index by 1
    except ValueError:
...
def dash_admin_meetings(req):
    ...
    if profile.is_member_of('Admins') or profile.is_member_of('UNICEF Officials') or profile.is_member_of(
            'Ministry Officials'):
...
def dash_district_meetings(req, district_name):
    ...
    for school_name, school_id, smc_connection in schools_data:
        school_container[(school_name, school_id)] = preprocess_response(
            [r.eav.poll_number_value for r in p.responses.filter(contact__connection__pk=smc_connection,
                                                                 date__range=[getattr(settings, 'SCHOOL_TERM_START'),
...
def head_teachers_male(locations, get_time=datetime.datetime.now):
    ...
    return {'m_head_t_week': male_d1, 'm_head_t_data': m_head_t_data, 'm_head_t_week_before': male_d2,
...
def view_generator(req,
    ...
    if req.method == 'POST':
        ...
        if time_range_form.is_valid():
            ...
            if profile.is_member_of('Ministry Officials') or profile.is_member_of('Admins') \
                ...
                location_schools = School.objects.filter(
                    pk__in=EnrolledDeployedQuestionsAnswered.objects.select_related(). \
    ...
    # NO POST data sent!
        ...
        sw = date_weeks[0][0]
        date_weeks.append(
            previous_calendar_week(t=dateutils.increment(datetime.datetime(sw.year, sw.month, sw.day, 10), weeks=-1)))

        location_data = []
        schools_temp = School.objects.select_related(). \
            filter(pk__in= \
            EnrolledDeployedQuestionsAnswered.objects.select_related().filter(
                school__location__in=locations).values_list('school__pk', flat=True))
        ...
        if profile.is_member_of('Ministry Officials') or profile.is_member_of('Admins') \
            ...
            location_schools = School.objects.select_related().filter(
                pk__in=EnrolledDeployedQuestionsAnswered.objects.select_related(). \
        ...
        if profile.is_member_of('Ministry Officials') or profile.is_member_of('Admins') or profile.is_member_of(
            ...
            x = {'url_name': url_name_district,
                 'headings': ['District', 'Current week', 'Previous week', 'Percentage difference']}
        else:
...
def admin_dashboard(request):
    if request.user.get_profile().is_member_of('Ministry Officials') or request.user.get_profile().is_member_of(
            'Admins') \
...
class NationalStatistics(TemplateView):
    ...
    def compute_percent(self, reps, groups=groups):
        all_reporters = EmisReporter.objects.filter(groups__name__in=groups).exclude(
            connection__in=Blacklist.objects.all()).exclude(schools=None)
        try:
    ...
    def get_context_data(self, **kwargs):
        ...
        if profile.is_member_of('Ministry Officials') or profile.is_member_of('Admins') or profile.is_member_of(
            ...
            districts = Location.objects.filter(type="district"). \
                filter(name__in=EmisReporter.objects.exclude(schools=None).exclude(
                connection__in=Blacklist.objects.values_list('connection', flat=True)).values_list(
                'reporting_location__name', flat=True))
            reps = EmisReporter.objects.select_related().filter(
                groups__name__in=['Teachers', 'Head Teachers', 'SMC', 'GEM', 'Other Reporters'],
                connection__in=Message.objects. \
                    filter(date__range=get_week_date(depth=2)[1]).values_list('connection', flat=True)).exclude(
                schools=None).exclude(connection__in=Blacklist.objects.values_list('connection', flat=True))

            district_schools = [
                (district,
                 School.objects.filter(pk__in=EmisReporter.objects.exclude(schools=None).exclude(
                     connection__in=Blacklist.objects.values_list('connection', flat=True)). \
                     filter(reporting_location__name=district.name).distinct().values_list('schools__pk',
                                                                                           flat=True)).count())
            ...
            schools = School.objects.filter(pk__in=EmisReporter.objects.exclude(schools=None). \
                exclude(connection__in=Blacklist.objects.values_list('connection', flat=True)).distinct().values_list(
                'schools__pk', flat=True))
            context['school_count'] = schools.count()
            total_schools = 0
            for district in districts:
                s_count = School.objects.filter(pk__in=EmisReporter.objects.exclude(schools=None).exclude(
                    connection__in=Blacklist.objects.values_list('connection', flat=True)). \
                    filter(reporting_location__name=district.name).distinct().values_list('schools__pk',
                                                                                          flat=True)).count()
            ...
            district_active = [
                (
                    district,
                    self.compute_percent(reps.filter(reporting_location__pk=district.pk), groups=['Head Teachers'])
                )
                for district in districts
            ]
            ...
            school_active = [
                (school, self.compute_percent(active_school_reporters.filter(schools__pk=school.pk),
                                              groups=['Head Teachers', 'Teachers', 'GEM', 'SMC', 'Other Reporters']))
                for school in schools
            ]

            school_active.sort(key=operator.itemgetter(1), reverse=True)
            context['school_active_count'] = School.objects.filter(
                pk__in=active_school_reporters.values_list('schools__pk', flat=True)).count()
...
def get_term_range(term):
    if term == 'first':
        return [getattr(settings, 'FIRST_TERM_BEGINS'),
                dateutils.increment(getattr(settings, 'FIRST_TERM_BEGINS'), weeks=12)]
    if term == 'second':
        return [getattr(settings, 'SECOND_TERM_BEGINS'),
                dateutils.increment(getattr(settings, 'SECOND_TERM_BEGINS'), weeks=12)]
    if term == 'third':
        return [getattr(settings, 'THIRD_TERM_BEGINS'),
                dateutils.increment(getattr(settings, 'THIRD_TERM_BEGINS'), weeks=12)]
...
class CapitationGrants(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if authorized_user:
            ...
            all_responses = cg.responses_by_category().exclude(response__in=unknown_unknowns).filter(
...
def monthly_violence_cases_for(poll, context_vars, locations):
    ...
    violence_cases_for_current_month = get_numeric_data_for_location(poll, locations, current_month)

    current_month_total = 0
    previous_month_total = 0

    violence_current_month_value = 0
    violence_previous_month_value = 0

    for location in locations:

        if location.id in violence_cases_for_current_month:
            current_month_total += violence_cases_for_current_month[location.id]
            violence_current_month_value = violence_cases_for_current_month[location.id]

        if location.id in violence_cases_for_previous_month:
            previous_month_total += violence_cases_for_previous_month[location.id]
            violence_previous_month_value = violence_cases_for_previous_month[location.id]

        violence_cases.append(
            (location.name, (violence_current_month_value, violence_previous_month_value, location)))
...
def violence_details_dash(req):
    ...
    try:
        context_vars['sch_reporting_percentage'] = 100 * (
            school_report_count / (float(len(get_month_day_range(datetime.datetime.now(),
                                                                 depth=2))) * school_report_count))
    except ZeroDivisionError:
        context_vars['sch_reporting_percentage'] = 0

    try:
        context_vars['gem_reporting_percentage'] = 100 * (
            gem_report_count / (float(len(get_month_day_range(datetime.datetime.now(),
    ...
    monthly_violence_data_girls = ';'.join(
        [str(item[0]) + '-' + str(item[1]) for item in zip(girls_violence_month, girls_violence_data)])
    monthly_violence_data_boys = ';'.join(
        [str(item[0]) + '-' + str(item[1]) for item in zip(boys_violence_month, boys_violence_data)])
    monthly_violence_data_reported = ';'.join(
        [str(item[0]) + '-' + str(item[1]) for item in zip(reported_violence_month, reported_violence_data)])
    ...
    context_vars[
        'schools_responding_to_all_questions'] = total_number_of_schools_that_responded_to_all_violence_questions(
        locations, current_month)
...
def __get_responses_for(poll_name):
    responses_for_all_questions = Response.objects.filter(
        poll__name__in=["edtrac_violence_girls", "edtrac_violence_boys", "edtrac_violence_reported"])
...
class DistrictViolenceDetails(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        location = Location.objects.filter(type="district").get(
            pk=int(self.kwargs.get('pk'))) or self.request.user.get_profile().location
...
class AttendanceAdminDetails(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if profile.is_member_of("Admins") or profile.is_member_of("Ministry Officials") or profile.is_member_of(
                'UNICEF Officials'):
            names = list(
                set(EmisReporter.objects.exclude(reporting_location=None).filter(reporting_location__type="district"). \
...
class MealsAdminDetails(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['school_meals_reports'] = get_count_response_to_polls(
...
def deo_dashboard(req):
    ...
    return render_to_response("education/deo/deo_dashboard.html", generate_dashboard_vars(location=location),
                              RequestContext(req))


@login_required
def quitters(req):
    quitters = EmisReporter.objects.filter(
...
def alert_messages(req):
    ...
    erroneous_messages = all_messages_filtered.filter(poll_responses=None) | all_messages_filtered.filter(
        poll_responses__has_errors=True)
    minimum_length = 20
    unsolicited_messages = erroneous_messages.filter(text__regex=r'^.{' + str(minimum_length) + ',}$')
    alerts = []
    for message in unsolicited_messages:
        phone_numbers = ','.join(
            message.connection.contact.emisreporter.connection_set.all().values_list('identity', flat=True))
...
def audit_trail(req):
    ...
    if profile.is_member_of('Admins') or profile.is_member_of('UNICEF Officials'):
        ...
        revisions = Revision.objects.exclude(comment='').order_by('-date_created').values_list('user__username',
                                                                                               'comment',
                                                                                               'date_created')
    else:
...
def boysp3_district_attd_detail(req, location_id):
    ...
    school_data, existing_data = view_stats_by_school(location_id, 'edtrac_boysp3_enrollment',
...
def boysp6_district_attd_detail(req, location_id):
    ...
    school_data, existing_data = view_stats_by_school(location_id, 'edtrac_boysp6_enrollment',
...
def girlsp3_district_attd_detail(req, location_id):
    ...
    school_data, existing_data = view_stats_by_school(location_id, 'edtrac_girlsp3_enrollment',
...
def girlsp6_district_attd_detail(req, location_id):
    ...
    school_data, existing_data = view_stats_by_school(location_id, 'edtrac_girlsp6_enrollment',
...
def female_t_district_attd_detail(req, location_id):
    ...
    school_data, existing_data = view_stats_by_school(location_id, 'edtrac_f_teachers_deployment',
...
def male_t_district_attd_detail(req, location_id):
    ...
    school_data, existing_data = view_stats_by_school(location_id, 'edtrac_m_teachers_deployment',
...
def boys_p3_attendance(req, **kwargs):
    ...
    if profile.is_member_of('Ministry Officials') or profile.is_member_of('Admins') or profile.is_member_of(
        ...
        locations = Location.objects.exclude(type="country").filter(type="district", name__in= \
            EmisReporter.objects.select_related().distinct().values_list('reporting_location__name',
        ...
        for school in schools:
            ...
            for d in dates:
                temp.extend(
                    return_absent('edtrac_boysp3_attendance', 'edtrac_boysp3_enrollment', school=school, date_week=d))
            to_ret.append(temp)
...
def male_head_teacher_attendance(req):
    ...
    for school in schools:
        ...
        data = poll_response_sum(Poll.objects.get(name="edtrac_head_teachers_attendance"), month_filter='weekly',
...
def female_head_teacher_attendance(req):
    ...
    if profile.is_member_of('Ministry Officials') or profile.is_member_of('Admins') or profile.is_member_of(
            'UNICEF Officials'):
        schools = School.objects.filter(location__name__in=EmisReporter.objects.distinct(). \
            select_related().filter(reporting_location__type='district').values_list('reporting_location__name',
                                                                                     flat=True))
...
def time_range_head_teachers(req):
    ...
    time_range_form = ResultForm()
    locations = Location.objects.filter(type='district').filter(
        pk__in=EmisReporter.objects.values_list('reporting_location__pk', flat=True))
    ...
    if req.method == 'POST':
        ...
        if time_range_form.is_valid():
            ...
            if req.user.get_profile().is_member_of('Ministry Officials') or \
                    req.user.get_profile().is_member_of('Admins') or req.user.get_profile().is_member_of(
                    'UNICEF Officials'):
                schools_temp = School.objects.select_related(). \
                ...
                for location in locations:
                    ...
                    for d in date_weeks:
                        ...
                        for school in location_schools:
                            deployed = poll_responses_term('edtrac_f_teachers_deployment', belongs_to='schools',
                                                           school=school)
            ...
            return render_to_response('education/timeslider_base.html', {'form': time_range_form,
                                                                         'url_name': "female-teacher-district-attd-detail",
                                                                         'title': 'Male Teacher Absenteeism'},
                                      RequestContext(req))
    else:
        ...
        temp_date = datetime.datetime(date_weeks[0][0].year, date_weeks[0][0].month,
                                      date_weeks[0][0].day) - datetime.timedelta(days=1)
        ...
        if profile.is_member_of('Ministry Officials') or profile.is_member_of('Admins') or profile.is_member_of(
            ...
            x = {'url_name': "female-teacher-district-attd-detail",
                 "headings": ["District", "Current Week", "Previous Week", "Percentage Change"]}
        else:
            x = {'url_name': "school-detail",
                 "headings": ["School", "Current Week", "Previous Week", "Percentage Change"]}
...
def add_connection(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            _reload_whitelists()
            #            time.sleep(2)
            return render_to_response('education/partials/addnumbers_row.html',
                                      {'object': connections, 'selectable': False}, RequestContext(request))
...
def delete_connection(request, connection_id):
    ...
    connection.delete()
    _reload_whitelists()
    return render_to_response("education/partials/connection_view.html", {'object': connection.contact},
                              context_instance=RequestContext(request))
...
def comments(req):
    ...
    if profile.is_member_of('Admins') or profile.is_member_of('Ministry Officials') or profile.is_member_of(
        ...
        comments = [(r.get_commentable_display(), r.comment, r.user, r.get_reporting_period_display(),
                     get_range_on_date(r.reporting_period, r))
                    for r in
                    ReportComment.objects.filter(user__profile__location=profile.location).order_by('-report_date')]
...
def new_comment(req):
    ...
    if req.method == 'POST':
        ...
        return render_to_response('education/partials/new_comment.html', {'form': report_comment_form},
                                  RequestContext(req))
...
def edit_reporter(request, reporter_pk):
    ...
    if request.method == 'POST':
        ...
        if reporter_form.is_valid():
            ...
            if reporter.default_connection and reporter_group_name != old_reporter_group:
...
def school_detail(request, school_id):
    ...
    for month_range in month_ranges:
        ...
        monthly_data_teachers.append(
            [return_absent_month(
                'edtrac_' + '%s' % slug + '_attendance',
                'edtrac_' + '%s' % slug + '_deployment', month_range=month_range, school=school) for slug in
             slug_list_tr])
        ...
        if user_form.is_valid():
            with reversion.create_revision():
                ...
                try:
                    ...
                    UserProfile.objects.create(name=user.first_name, user=user,
                                               role=Role.objects.get(pk=user_form.cleaned_data['groups'][0].pk),
...
def htattendance(request, start_date=None, end_date=None, district_id=None):
    ...
    smc_htpresent = Poll.objects.get(name='emis_absence').responses.exclude(has_errors=True) \
        ...
        .filter(message__connection__contact__emisreporter__reporting_location__in=user_location.get_descendants(
        include_self=True).all()) \
...
def gem_htattendance(request, start_date=None, end_date=None, district_id=None):
    ...
    gem_htpresent = XFormSubmission.objects.filter(xform__keyword='gemteachers').exclude(has_errors=True) \
        ...
        .filter(connection__contact__emisreporter__reporting_location__in=user_location.get_descendants(
        include_self=True).all()) \
        .order_by('created')
...
def _format_reporters(reporters):
    return [[r.id, r.name, _get_identity(r), r.reporting_location.type.name, r.reporting_location.name,
             ", ".join(r.schools.values_list('name', flat=True))] for r in reporters]
...
def edit_sub_county_reporters(request):
    ...
    if request.method == 'GET':
        ...
        return render_to_response('education/admin/edit_sub_county_reporters.html', {'reporters': sub_county_reporters},
                                  RequestContext(request))
...
def _format_messages(messages):
    return [[m, m.connection.contact.reporting_location, _get_school(m), m.connection.contact, m.connection, m.date] for
            m in messages]
