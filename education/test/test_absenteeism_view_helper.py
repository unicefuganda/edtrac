...
from education.absenteeism_view_helper import get_responses_over_depth, get_responses_by_location
...
class TestAbsenteeismViewHelper(TestAbsenteeism):
    def test_should_return_sum_over_districts(self):
        ...
        result_absent , result_enrolled = get_responses_over_depth(self.p3_boys_absent_poll.name,self.p3_boys_enrolled_poll.name, locations, 4)
    ...
    def test_should_ignore_locations_if_no_response_found(self):
        with patch('education.absenteeism_view_helper.get_responses_over_depth') as method_mock :
            ...
            get_responses_by_location(self.admin_user.get_profile(),self.p3_boys_absent_poll.name, self.p3_boys_enrolled_poll.name)
            method_mock.assert_called_with(self.p3_boys_absent_poll.name,self.p3_boys_enrolled_poll.name, [], 4)
    ...
    def test_should_give_result_for_p3_boys_poll(self):
        ...
        with patch('education.absenteeism_view_helper.get_responses_over_depth') as method_mock:
            method_mock.return_value = [],[]
            get_responses_by_location(self.admin_user.get_profile(),self.p3_boys_absent_poll.name, self.p3_boys_enrolled_poll.name)
            method_mock.assert_called_with(self.p3_boys_absent_poll.name,self.p3_boys_enrolled_poll.name, [self.kampala_district], 4)
    ...
    def test_should_give_result_for_p3_boys_poll_at_location(self):
        ...
        with patch('education.absenteeism_view_helper.get_responses_over_depth') as method_mock:
            ...
            get_responses_by_location(UserProfile(location=self.gulu_district, role=test_role),
                                      self.p3_boys_absent_poll.name,self.p3_boys_enrolled_poll.name)
            method_mock.assert_called_with(self.p3_boys_absent_poll.name,self.p3_boys_enrolled_poll.name, [self.gulu_district], 4)

    def test_should_aggregate_data_over_time_period(self):
        test_role = create_group('test')
        with patch('education.absenteeism_view_helper.get_responses_over_depth') as method_mock:
            method_mock.return_value = [],[]
            get_responses_by_location(UserProfile(location=self.gulu_district, role=test_role),
