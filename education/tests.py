...
class ModelTest(TestCase): #pragma: no cover
    ...
    def setUp(self):
        ...
        script_p3.steps.add(ScriptStep.objects.create(
            ...
            order=1,
            rule=ScriptStep.WAIT_MOVEON,
            # for polls, this likely means a poll whose answer we aren't particularly concerned with
    ...
    def elapseTime2(self, progress, seconds):
        ...
        try:
            session = ScriptSession.objects.get(connection=progress.connection, script__slug=progress.script.slug,
                                                end_time=None)
    ...
    def testBasicAutoReg(self):
        ...
        self.assertListEqual(
            list(ScriptProgress.objects.filter(connection=self.connection).values_list('script__slug', flat=True)),
            ['edtrac_autoreg', 'edtrac_teachers_weekly'])
    ...
    def testBasicQuit(self):
        self.register_reporter('1')
        ScriptProgress.objects.filter(script__slug='edtrac_autoreg',
                                      connection=self.connection).delete() #script always deletes a connection on completion of registration
        #quit system
        self.fake_incoming('quit')
        self.assertEquals(Message.objects.filter(direction='O').order_by('-date')[0].text,
                          "Thank you for your contribution to EduTrac. To rejoin the system, send join to 6200")
    ...
    def testQuitChangeGroup(self):
        ...
        Script.objects.filter(slug='edtrac_autoreg').update(
            enabled=True) #First enable the autoreg script if its not enabled
        self.fake_incoming('join')
        ...
        self.assertEquals(Message.objects.filter(direction='O').order_by('-date')[0].text,
                          Script.objects.get(slug='edtrac_autoreg').steps.get(order=0).poll.question)
        self.fake_incoming('2')
        script_prog = ScriptProgress.objects.get(connection=self.connection, script__slug='edtrac_autoreg')
        self.elapseTime2(script_prog, 3601)
        call_command('check_script_progress', e=8, l=24)
        self.assertEquals(Message.objects.filter(direction='O').order_by('-date')[0].text,
                          Script.objects.get(slug='edtrac_autoreg').steps.get(order=1).poll.question)
        self.assertNotEquals(Message.objects.filter(direction='O').order_by('-date')[0].text,
                             Script.objects.get(slug='edtrac_autoreg').steps.get(order=2).poll.question)
        self.fake_incoming('Male')
        script_prog = ScriptProgress.objects.get(connection=self.connection, script__slug='edtrac_autoreg')
        self.elapseTime2(script_prog, 3601)
        call_command('check_script_progress', e=8, l=24)
        self.assertEquals(Message.objects.filter(direction='O').order_by('-date')[0].text,
                          Script.objects.get(slug='edtrac_autoreg').steps.get(order=3).poll.question)
        self.fake_incoming('Kampala')
        script_prog = ScriptProgress.objects.get(connection=self.connection, script__slug='edtrac_autoreg')
        self.elapseTime2(script_prog, 3601)
        call_command('check_script_progress', e=8, l=24)
        self.assertEquals(Message.objects.filter(direction='O').order_by('-date')[0].text,
                          Script.objects.get(slug='edtrac_autoreg').steps.get(order=4).poll.question)
        self.fake_incoming('Kampala')
        script_prog = ScriptProgress.objects.get(connection=self.connection, script__slug='edtrac_autoreg')
        self.elapseTime2(script_prog, 3601)
        call_command('check_script_progress', e=8, l=24)
        self.assertEquals(Message.objects.filter(direction='O').order_by('-date')[0].text,
                          Script.objects.get(slug='edtrac_autoreg').steps.get(order=5).poll.question)
        self.fake_incoming('St. Marys')
        script_prog = ScriptProgress.objects.get(connection=self.connection, script__slug='edtrac_autoreg')
        self.elapseTime2(script_prog, 3601)
        call_command('check_script_progress', e=8, l=24)
        self.assertEquals(Message.objects.filter(direction='O').order_by('-date')[0].text,
                          Script.objects.get(slug='edtrac_autoreg').steps.get(order=6).poll.question)
        self.fake_incoming('test mctester')
        script_prog = ScriptProgress.objects.get(connection=self.connection, script__slug='edtrac_autoreg')
        self.elapseTime2(script_prog, 3601)
        call_command('check_script_progress', e=8, l=24)
        self.assertEquals(Message.objects.filter(direction='O').order_by('-date')[0].text,
                          Script.objects.get(slug='edtrac_autoreg').steps.get(order=7).message)
        self.elapseTime2(script_prog, 1)
        call_command('check_script_progress', e=8, l=24)
        self.assertEquals(EmisReporter.objects.count(),
                          1) #we still have only one user in the system, same user who quit and rejoined
        self.assertEquals(EmisReporter.objects.all()[0].groups.count(), 1) #user retains on one group
        self.assertEquals(EmisReporter.objects.all()[0].groups.all()[0].name,
                          Group.objects.filter(name='Head Teachers')[0].name) #group has changed
    ...
    def testDoubleReg(self):
        ...
        self.fake_incoming('join')
        self.assertEquals(Message.objects.filter(direction='O').order_by('-date')[0].text,
                          "Your registration is not complete yet, you do not need to 'Join' again.")
        ...
        self.fake_incoming('join')
        self.assertEquals(Message.objects.filter(direction='O').order_by('-date')[0].text,
                          "You are already in the system and do not need to 'Join' again.")
    ...
    def testQuitIncompleteRegistration(self):
        ...
        self.fake_incoming('join')
        self.assertEquals(
            ScriptProgress.objects.filter(script__slug='edtrac_autoreg', connection=self.connection).count(), 1)

        #then quit
        self.fake_incoming('quit')
        self.assertEquals(Blacklist.objects.count(), 0)
        #notify user to first complete current registration
        self.assertEquals(Message.objects.all().order_by('-date')[0].text,
                          'Your registration is not complete, you can not quit at this point')
    ...
    def testGemAutoReg(self):
        ...
        self.assertListEqual(
            list(ScriptProgress.objects.filter(connection=self.connection).values_list('script__slug', flat=True)),
            ['edtrac_autoreg', 'edtrac_gem_monthly'])
    ...
    def testRoleAssignment(self):
        ...
        session =
        ScriptSession.objects.filter(script=prog.script, connection__identity='8675319').order_by('-end_time')[0]
        role_poll = prog.script.steps.get(poll__name='edtrac_role').poll
        role = find_best_response(session, role_poll)
        group = match_group_response(session, role, role_poll)
        ...
        session =
        ScriptSession.objects.filter(script=prog.script, connection__identity='8675329').order_by('-end_time')[0]
        role_poll = prog.script.steps.get(poll__name='edtrac_role').poll
        role = find_best_response(session, role_poll)
        group = match_group_response(session, role, role_poll)
        ...
        session =
        ScriptSession.objects.filter(script=prog.script, connection__identity='8675339').order_by('-end_time')[0]
        role_poll = prog.script.steps.get(poll__name='edtrac_role').poll
        role = find_best_response(session, role_poll)
        group = match_group_response(session, role, role_poll)
        ...
        session =
        ScriptSession.objects.filter(script=prog.script, connection__identity='8675349').order_by('-end_time')[0]
        role_poll = prog.script.steps.get(poll__name='edtrac_role').poll
        role = find_best_response(session, role_poll)
        group = match_group_response(session, role, role_poll)
        ...
        session =
        ScriptSession.objects.filter(script=prog.script, connection__identity='8675359').order_by('-end_time')[0]
        role_poll = prog.script.steps.get(poll__name='edtrac_role').poll
        role = find_best_response(session, role_poll)
        group = match_group_response(session, role, role_poll)
        ...
        session =
        ScriptSession.objects.filter(script=prog.script, connection__identity='8675369').order_by('-end_time')[0]
        role_poll = prog.script.steps.get(poll__name='edtrac_role').poll
        role = find_best_response(session, role_poll)
        group = match_group_response(session, role, role_poll)
    ...
    def testWeeklyTeacherPolls(self):
        ...
        self.assertEquals(Message.objects.filter(direction='O').order_by('-date')[0].text,
                          Script.objects.get(slug='edtrac_p3_teachers_weekly').steps.get(order=0).poll.question)
        self.fake_incoming('40')
        self.assertEquals(Message.objects.filter(direction='I').order_by('-date')[0].application, 'script')
        self.assertEquals(
            Script.objects.get(slug='edtrac_p3_teachers_weekly').steps.get(order=0).poll.responses.all().order_by(
                '-date')[0].eav.poll_number_value, 40)
        self.elapseTime2(prog, 61)
        prog = ScriptProgress.objects.get(script__slug='edtrac_p3_teachers_weekly', connection=self.connection)
        check_progress(prog.script)
        self.assertEquals(Message.objects.filter(direction='O').order_by('-date')[0].text,
                          Script.objects.get(slug='edtrac_p3_teachers_weekly').steps.get(order=1).poll.question)
        self.fake_incoming('55girls')
        self.fake_incoming('65girls') # proof that the last instantaneous response is not equal to 65
        self.assertEquals(
            Script.objects.get(slug='edtrac_p3_teachers_weekly').steps.get(order=1).poll.responses.all().order_by(
                '-date')[0].eav.poll_number_value, 55)
        self.assertNotEquals(
            Script.objects.get(slug='edtrac_p3_teachers_weekly').steps.get(order=1).poll.responses.all().order_by(
                '-date')[0].eav.poll_number_value, 65)
        self.elapseTime2(prog, 61)
        prog = ScriptProgress.objects.get(script__slug='edtrac_p3_teachers_weekly', connection=self.connection)
        check_progress(prog.script)
        self.assertEquals(Message.objects.filter(direction='O').order_by('-date')[0].text,
                          Script.objects.get(slug='edtrac_p3_teachers_weekly').steps.get(order=2).poll.question)
        self.fake_incoming('3')
        self.assertEquals(
            Script.objects.get(slug='edtrac_p3_teachers_weekly').steps.get(order=2).poll.responses.all().order_by(
                '-date')[0].eav.poll_number_value, 3)
        self.elapseTime2(prog, 61)
        prog = ScriptProgress.objects.get(script__slug='edtrac_p3_teachers_weekly', connection=self.connection)
        check_progress(prog.script)
    ...
    def testWeeklyHeadTeacherPolls(self):
        ...
        Script.objects.filter(slug__in=['edtrac_head_teachers_weekly', 'edtrac_head_teachers_monthly',
                                        'edtrac_head_teachers_termly']).update(enabled=True)
        ...
        check_progress(prog.script);
        check_progress(prog.script);
        check_progress(prog.script); # multiple check_progresses for Head Teacher b'se of question skip
        self.assertEquals(Message.objects.filter(direction='O').order_by('-date')[0].text,
                          Script.objects.get(slug='edtrac_head_teachers_weekly').steps.get(order=0).poll.question)
        self.fake_incoming('6')
        self.assertEquals(Message.objects.filter(direction='I').order_by('-date')[0].application, 'script')
        self.assertEquals(
            Script.objects.get(slug='edtrac_head_teachers_weekly').steps.get(order=0).poll.responses.all().order_by(
                '-date')[0].eav.poll_number_value, 6)
        self.elapseTime2(prog, 61)
        prog = ScriptProgress.objects.get(script__slug='edtrac_head_teachers_weekly', connection=self.connection)
        check_progress(prog.script)
        self.assertEquals(Message.objects.filter(direction='O').order_by('-date')[0].text,
                          Script.objects.get(slug='edtrac_head_teachers_weekly').steps.get(order=1).poll.question)
        self.fake_incoming('5 male teachers')
        self.assertEquals(
            Script.objects.get(slug='edtrac_head_teachers_weekly').steps.get(order=1).poll.responses.all().order_by(
                '-date')[0].eav.poll_number_value, 5)
        self.elapseTime2(prog, 61)
        prog = ScriptProgress.objects.get(script__slug='edtrac_head_teachers_weekly', connection=self.connection)
        check_progress(prog.script)
        self.assertEquals(ScriptProgress.objects.get(connection=self.connection, script=prog.script).__unicode__(),
                          'Not Started')
    ...
    def testWeeklySMCPolls(self):
        ...
        Script.objects.filter(slug__in=['edtrac_smc_weekly', 'edtrac_smc_monthly', 'edtrac_smc_termly']).update(
            enabled=True)
        ...
        check_progress(prog.script)
        self.assertEquals(Message.objects.filter(direction='O').order_by('-date')[0].text,
                          Script.objects.get(slug='edtrac_smc_weekly').steps.get(order=0).poll.question)
        ...
        self.assertEquals(ResponseCategory.objects.get(response__poll__name=poll.name, category=yes_category).response,
                          response)
    ...
    def testMonthlyHeadTeacherPolls(self):
        ...
        check_progress(prog.script)
        self.assertEquals(Message.objects.filter(direction='O').order_by('-date')[0].text,
                          Script.objects.get(slug='edtrac_head_teachers_monthly').steps.get(order=0).poll.question)
        self.fake_incoming('5')
        self.assertEquals(Message.objects.filter(direction='I').order_by('-date')[0].application, 'script')
        self.assertEquals(
            Script.objects.get(slug='edtrac_head_teachers_monthly').steps.get(order=0).poll.responses.all().order_by(
                '-date')[0].eav.poll_number_value, 5)
        self.elapseTime2(prog, 61)
        prog = ScriptProgress.objects.get(script__slug='edtrac_head_teachers_monthly', connection=self.connection)
        check_progress(prog.script)
        self.assertEquals(Message.objects.filter(direction='O').order_by('-date')[0].text,
                          Script.objects.get(slug='edtrac_head_teachers_monthly').steps.get(order=1).poll.question)
        self.fake_incoming('25%')
        self.assertEquals(Message.objects.filter(direction='I').order_by('-date')[0].application, 'script')
        self.assertEquals(
            Script.objects.get(slug='edtrac_head_teachers_monthly').steps.get(order=1).poll.responses.all().order_by(
                '-date')[0].eav.poll_number_value, 25)
        self.elapseTime2(prog, 61)
        prog = ScriptProgress.objects.get(script__slug='edtrac_head_teachers_monthly', connection=self.connection)
        check_progress(prog.script)
        self.assertEquals(ScriptProgress.objects.get(connection=self.connection, script=prog.script).__unicode__(),
                          'Not Started')
        self.assertEquals(ScriptProgress.objects.get(connection=self.connection, script=prog.script).time.hour, 10)
        seconds_to_nextprog = self.total_seconds(
            ScriptProgress.objects.get(connection=self.connection, script=prog.script).time - datetime.datetime.now())
    ...
    def testMonthlySMCPolls(self):
        ...
        self.assertEquals(
            Script.objects.get(slug='edtrac_smc_monthly').steps.get(order=0).poll.responses.all().order_by('-date')[
                0].eav.poll_number_value, 50)
        prog = ScriptProgress.objects.get(script__slug='edtrac_smc_monthly', connection=self.connection)
        self.elapseTime2(prog, 61)
        prog = ScriptProgress.objects.get(script__slug='edtrac_smc_monthly', connection=self.connection)
        check_progress(prog.script)
    ...
    def testMonthlyGEMPolls(self):
        ...
        check_progress(prog.script)
        self.assertEquals(Message.objects.filter(direction='O').order_by('-date')[0].text,
                          Script.objects.get(slug='edtrac_gem_monthly').steps.get(order=0).poll.question)
        self.fake_incoming('St Peters PS, St John PS')
        self.assertEquals(
            Script.objects.get(slug='edtrac_gem_monthly').steps.get(order=0).poll.responses.all().order_by('-date')[
                0].eav.poll_text_value, 'St Peters PS, St John PS')
        self.elapseTime2(prog, 1)
        prog = ScriptProgress.objects.get(script__slug='edtrac_gem_monthly', connection=self.connection)
        check_progress(prog.script)
        self.assertEquals(Message.objects.filter(direction='O').order_by('-date')[0].text,
                          Script.objects.get(slug='edtrac_gem_monthly').steps.get(order=1).poll.question)
        self.fake_incoming('St Peters PS, St John PS')
        self.assertEquals(
            Script.objects.get(slug='edtrac_gem_monthly').steps.get(order=1).poll.responses.all().order_by('-date')[
                0].eav.poll_text_value, 'St Peters PS, St John PS')
        self.elapseTime2(prog, 1)
        prog = ScriptProgress.objects.get(script__slug='edtrac_gem_monthly', connection=self.connection)
        check_progress(prog.script)
        self.assertEquals(Message.objects.filter(direction='O').order_by('-date')[0].text,
                          Script.objects.get(slug='edtrac_gem_monthly').steps.get(order=2).poll.question)
        self.fake_incoming('50')
        self.assertEquals(
            Script.objects.get(slug='edtrac_gem_monthly').steps.get(order=2).poll.responses.all().order_by('-date')[
                0].eav.poll_number_value, 50)
        self.elapseTime2(prog, 61)
        prog = ScriptProgress.objects.get(script__slug='edtrac_gem_monthly', connection=self.connection)
        check_progress(prog.script)
    ...
    def testTermlyHeadTeacherPolls(self):
        ...
        check_progress(prog.script)
        self.assertEquals(Message.objects.filter(direction='O').order_by('-date')[0].text,
                          Script.objects.get(slug='edtrac_head_teachers_termly').steps.get(order=0).poll.question)
        self.fake_incoming('85')
        self.assertEquals(Message.objects.filter(direction='I').order_by('-date')[0].application, 'script')
        print Message.objects.all()
        self.assertEquals(
            Script.objects.get(slug='edtrac_head_teachers_termly').steps.get(order=0).poll.responses.all().order_by(
                '-date')[0].eav.poll_number_value, 85)
        self.elapseTime2(prog, 61)
        prog = ScriptProgress.objects.get(script__slug='edtrac_head_teachers_termly', connection=self.connection)
        check_progress(prog.script)
        self.assertEquals(Message.objects.filter(direction='O').order_by('-date')[0].text,
                          Script.objects.get(slug='edtrac_head_teachers_termly').steps.get(order=1).poll.question)
        self.fake_incoming('25')
        self.assertEquals(
            Script.objects.get(slug='edtrac_head_teachers_termly').steps.get(order=1).poll.responses.all().order_by(
                '-date')[0].eav.poll_number_value, 25)
        self.elapseTime2(prog, 61)
        prog = ScriptProgress.objects.get(script__slug='edtrac_head_teachers_termly', connection=self.connection)
        check_progress(prog.script)
        self.assertEquals(Message.objects.filter(direction='O').order_by('-date')[0].text,
                          Script.objects.get(slug='edtrac_head_teachers_termly').steps.get(order=2).poll.question)
        self.fake_incoming('25')
        self.assertEquals(
            Script.objects.get(slug='edtrac_head_teachers_termly').steps.get(order=2).poll.responses.all().order_by(
                '-date')[0].eav.poll_number_value, 25)
        self.elapseTime2(prog, 61)
        prog = ScriptProgress.objects.get(script__slug='edtrac_head_teachers_termly', connection=self.connection)
        check_progress(prog.script)
        self.assertEquals(Message.objects.filter(direction='O').order_by('-date')[0].text,
                          Script.objects.get(slug='edtrac_head_teachers_termly').steps.get(order=3).poll.question)
        self.fake_incoming('25')
        self.assertEquals(
            Script.objects.get(slug='edtrac_head_teachers_termly').steps.get(order=3).poll.responses.all().order_by(
                '-date')[0].eav.poll_number_value, 25)
        self.elapseTime2(prog, 61)
        prog = ScriptProgress.objects.get(script__slug='edtrac_head_teachers_termly', connection=self.connection)
        check_progress(prog.script)
        self.assertEquals(Message.objects.filter(direction='O').order_by('-date')[0].text,
                          Script.objects.get(slug='edtrac_head_teachers_termly').steps.get(order=4).poll.question)
        self.fake_incoming('25')
        self.elapseTime2(prog, 61)
        prog = ScriptProgress.objects.get(script__slug='edtrac_head_teachers_termly', connection=self.connection)
        check_progress(prog.script)
        self.assertEquals(Message.objects.filter(direction='O').order_by('-date')[0].text,
                          Script.objects.get(slug='edtrac_head_teachers_termly').steps.get(order=5).poll.question)
        self.fake_incoming('25')
        self.assertEquals(
            Script.objects.get(slug='edtrac_head_teachers_termly').steps.get(order=5).poll.responses.all().order_by(
                '-date')[0].eav.poll_number_value, 25)
        self.elapseTime2(prog, 61)
        prog = ScriptProgress.objects.get(script__slug='edtrac_head_teachers_termly', connection=self.connection)
        check_progress(prog.script)
        self.assertEquals(Message.objects.filter(direction='O').order_by('-date')[0].text,
                          Script.objects.get(slug='edtrac_head_teachers_termly').steps.get(order=6).poll.question)
        self.fake_incoming('yes')
        self.assertEquals(
            Script.objects.get(slug='edtrac_head_teachers_termly').steps.get(order=6).poll.responses.all().order_by(
                '-date')[0].eav.poll_text_value, 'yes')
        self.elapseTime2(prog, 61)
        prog = ScriptProgress.objects.get(script__slug='edtrac_head_teachers_termly', connection=self.connection)
        check_progress(prog.script)
        self.assertEquals(ScriptProgress.objects.get(connection=self.connection, script=prog.script).__unicode__(),
                          'Not Started')
        #time checks
        self.assertEquals(ScriptProgress.objects.get(connection=self.connection, script=prog.script).time.date(),
                          d.date())
        # micro seconds make test fail
        self.assertEquals(ScriptProgress.objects.get(connection=self.connection, script=prog.script).time.time().hour,
                          d.time().hour)
    ...
    def testPollRetryRules(self):
        ...
        check_progress(prog.script)
        self.assertEquals(Message.objects.filter(direction='O').order_by('-date')[0].text,
                          Script.objects.get(slug='edtrac_teachers_weekly').steps.get(order=0).poll.question)
        check_progress(prog.script)
        ...
        self.assertEquals(Message.objects.filter(direction='O',
                                                 text=Script.objects.get(slug='edtrac_teachers_weekly').steps.get(
                                                     order=0).poll.question).count(), 1)
    ...
    def testRescheduleWeeklyPolls(self):
        ...
        self.assertEquals(
            ScriptProgress.objects.filter(script__slug__in=weekly_scripts.values_list('slug', flat=True))[0].time.year,
            datetime.datetime.now().year - 1)
        reschedule_weekly_polls('teachers')
        next_thursday = _next_thursday()
        self.assertEquals(
            ScriptProgress.objects.get(connection__identity='8675349', script__slug='edtrac_teachers_weekly').time,
            next_thursday)
        reschedule_weekly_polls('head teachers')
        self.assertEquals(
            ScriptProgress.objects.get(connection__identity='8675319', script__slug='edtrac_head_teachers_weekly').time,
            next_thursday)
        reschedule_weekly_polls('smc')
        self.assertEquals(
            ScriptProgress.objects.get(connection__identity='8675329', script__slug='edtrac_smc_weekly').time,
            next_thursday)
        for sp in ScriptProgress.objects.filter(script__slug__in=weekly_scripts.values_list('slug', flat=True)):
            self.elapseTime2(sp, 13 * 31 * 24 * 60 * 60)
    ...
    def testRescheduleTeacherWeeklyPolls(self):
        ...
        next_thursday = _next_thursday()
        self.assertEquals(
            ScriptProgress.objects.get(connection__identity='8675349', script__slug='edtrac_p3_teachers_weekly').time,
            next_thursday)
        reschedule_teacher_weekly_polls('Teachers')
        self.assertEquals(
            ScriptProgress.objects.get(connection__identity='8675319', script__slug='edtrac_p6_teachers_weekly').time,
            next_thursday)
        for sp in ScriptProgress.objects.filter(script__slug__in=weekly_scripts.values_list('slug', flat=True)):
            self.elapseTime2(sp, 13 * 31 * 24 * 60 * 60)
    ...
    def testRescheduleMonthlyPolls(self):
        ...
        for sp in ScriptProgress.objects.filter(script__slug__in=monthly_scripts.values_list('slug', flat=True)):
            self.elapseTime2(sp, 13 * 31 * 24 * 60 * 60)
        self.assertEquals(
            ScriptProgress.objects.filter(script__slug__in=monthly_scripts.values_list('slug', flat=True))[0].time.year,
            datetime.datetime.now().year - 1)
        reschedule_monthly_polls('head teachers')
        self.assertEquals(ScriptProgress.objects.get(connection__identity='8675319',
                                                     script__slug='edtrac_head_teachers_monthly').time.date(),
                          _date_of_monthday('last').date())
        reschedule_monthly_polls('smc')
        self.assertEquals(
            ScriptProgress.objects.get(connection__identity='8675329', script__slug='edtrac_smc_monthly').time.date(),
            _date_of_monthday(5).date())
        reschedule_monthly_polls('gem')
        self.assertEquals(
            ScriptProgress.objects.get(connection__identity='8675339', script__slug='edtrac_gem_monthly').time.date(),
            _date_of_monthday(20).date())
        for sp in ScriptProgress.objects.filter(script__slug__in=monthly_scripts.values_list('slug', flat=True)):
            self.elapseTime2(sp, 13 * 31 * 24 * 60 * 60)
    ...
    def testRescheduleTermlyPolls(self):
        ...
        self.assertEquals(ScriptProgress.objects.get(connection__identity='8675319',
                                                     script__slug='edtrac_head_teachers_termly').time.date(),
                          _next_term_question_date().date())
        reschedule_termly_polls('smc')
        self.assertEquals(
            ScriptProgress.objects.get(connection__identity='8675329', script__slug='edtrac_smc_termly').time.date(),
            _next_term_question_date(rght=True).date())
        for sp in ScriptProgress.objects.filter(script__slug__in=termly_scripts.values_list('slug', flat=True)):
            self.elapseTime2(sp, 13 * 31 * 24 * 60 * 60)
        ...
        self.assertEquals(
            ScriptProgress.objects.get(connection__identity='8675329', script__slug='edtrac_smc_termly').time.date(),
            datetime.datetime(2012, 4, 16).date())
        for sp in ScriptProgress.objects.filter(script__slug__in=termly_scripts.values_list('slug', flat=True)):
            self.elapseTime2(sp, 13 * 31 * 24 * 60 * 60)
        reschedule_termly_polls('all', '2012-4-17')
        self.assertEquals(ScriptProgress.objects.get(connection__identity='8675319',
                                                     script__slug='edtrac_head_teachers_termly').time.date(),
                          datetime.datetime(2012, 4, 17).date())
        self.assertEquals(
            ScriptProgress.objects.get(connection__identity='8675329', script__slug='edtrac_smc_termly').time.date(),
            datetime.datetime(2012, 4, 17).date())
    ...
    def testRescheduleMidTermPolls(self):
        ...
        self.assertEquals(ScriptProgress.objects.get(connection__identity='8675319',
                                                     script__slug='edtrac_head_teachers_midterm').time.date(),
                          _next_midterm().date())
        for sp in ScriptProgress.objects.filter(script__slug__in=midterm_scripts.values_list('slug', flat=True)):
            self.elapseTime2(sp, 13 * 31 * 24 * 60 * 60)
        ...
        self.assertEquals(ScriptProgress.objects.get(connection__identity='8675319',
                                                     script__slug='edtrac_head_teachers_midterm').time.date(),
                          datetime.datetime(2012, 10, 8).date())
        for sp in ScriptProgress.objects.filter(script__slug__in=midterm_scripts.values_list('slug', flat=True)):
            self.elapseTime2(sp, 13 * 31 * 24 * 60 * 60)
