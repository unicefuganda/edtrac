...
class ScriptProgressQuerySet(QuerySet):
    ...
    def expired(self, script, step):
        ...
        if not step.start_offset:
            ...
            num_tries = step.num_tries or 0
    
            return self.filter(step=step, script=script).filter(\
    ...
    def expire(self, script, step):
        self.filter(step__rule__in=[step.WAIT_GIVEUP, step.RESEND_GIVEUP, step.STRICT_GIVEUP]).giveup(script,step)
        self.exclude(step__rule__in=[step.WAIT_GIVEUP, step.RESEND_GIVEUP, step.STRICT_GIVEUP]).update(status=self.model.COMPLETE)
    ...
    def mass_text(self):
        ...
        for language in dict(settings.LANGUAGES).keys():
            ...
            if localized_progs.exists():
                ...
                messages = Message.mass_text(gettext_db(field=text,language=language), Connection.objects.filter(pk__in=localized_conns).distinct(), status='L')
        return True
