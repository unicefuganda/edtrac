...
class App (AppBase):
    ...
    def handle (self, message):
        ...
        #validation
        # the assumption is that messages that are sent in are not more than 1 valued responses
        # examples of such messages from the xform circa which isn't used much in EduTra: Teachers M 3 F 4
        if len(message.text.strip().lower().strip()) > 1:
            #TODO: handle situations where user is expected to send in a text with a response like ``I don't know``
            message.respond("You have sent an invalid message to EduTrac (6200). Please send only one value as a response to the question.")
            return
        else:
